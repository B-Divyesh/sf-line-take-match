# Copy audit — polish round 3

Audited 2026-08-28. Counts use whitespace-delimited words. No sentence exceeds
22 words and no banned marketing term appears. “Take list,” “take,” “approved
take,” and “line” are the only product nouns for those concepts.

## Landing route

| Copy | Words | Result |
| --- | ---: | --- |
| Skip to main content | 4 | Pass |
| Voice take comparison | 3 | Pass |
| Compare voice takes with an approved take. | 7 | Pass |
| For indie animators and game creators checking whether recorded character lines match. | 12 | Pass |
| Try it with sample data | 5 | Pass |
| Import audio takes | 3 | Pass |
| The demo opens three dialogue takes to compare. | 8 | Listed claim |
| Audio stays on this device | 5 | Listed claim |
| Works offline after the first visit | 6 | Listed claim |
| Free for 12 takes · Studio costs $19 once | 8 | Listed claims |
| Choose audio files your browser can play. | 7 | Pass |
| If another format fails, convert it to WAV and try again. | 11 | Pass |
| Filenames such as door-warning_take-03.wav group by line. | 7 | Listed claim |
| I have the performer’s consent and rights to review these recordings. | 10 | Pass |
| Your takes will appear here | 5 | Pass |
| Import two or more recordings of one line. | 8 | Pass |
| Choose the approved take, then listen to any measured differences. | 10 | Pass |
| Import a project backup | 4 | Pass |
| Group takes by filename | 4 | Listed claim |
| Choose the take to compare against | 6 | Pass |
| Flag a take and export CSV | 6 | Pass |
| Compare recorded takes | 3 | Pass |
| Import audio. | 2 | Pass |
| Filenames group takes for the same line. | 7 | Listed claim |
| Choose the approved take. | 4 | Pass |
| Compare level, pace, pauses, and pitch range. | 7 | Listed claim |
| Flag and export. | 3 | Pass |
| Add notes and download a CSV for your team. | 9 | Listed claim |
| Line Take Match does not transcribe, generate, or clone voices. | 10 | Listed claim |
| Measurements are review cues, never performance scores. | 7 | Listed claim |
| Free mode includes 12 takes and CSV exports. | 8 | Listed claim |
| Studio costs $19 once and adds unlimited takes and audio backups. | 11 | Listed claim |
| Compare recorded voice takes without uploading audio. | 7 | Listed claim |

## Demo first viewport and take actions

| Copy | Words | Result |
| --- | ---: | --- |
| Demo — sample data, nothing is saved to your take list | 10 | Listed claim |
| Reset demo | 2 | Pass |
| Start for real | 3 | Pass |
| Compare sample takes. | 3 | Pass |
| Hear the approved read, then one alternative. | 7 | Pass |
| Approved take | 2 | Status, not an action |
| Compare with | 2 | Pass |
| Play approved, then this take | 6 | Listed claim |
| 1 line | 2 | Pass |
| Remove review flag | 3 | Names result |
| Flag review | 2 | Names result |
| Manage Studio license | 3 | Names result |

## Studio and README

| Copy | Words | Result |
| --- | ---: | --- |
| Free mode compares up to 12 takes and includes CSV export. | 10 | Listed claim |
| Studio costs $19 once. | 4 | Listed claim |
| It adds unlimited takes and portable project backups. | 8 | Listed claim |
| Studio backups include audio and can restore a take list in another browser. | 13 | Listed claim |
| Studio checkout opens on Sociobot. | 5 | Listed claim |
| A revoked license no longer unlocks Studio. | 8 | Listed claim |
| See privacy and terms. | 4 | Pass |

## Terminology

| Concept | Term |
| --- | --- |
| Collection of imported recordings | take list |
| One recording | take |
| Baseline recording | approved take |
| Editable dialogue grouping | line |
| Measured difference needing a listen | review cue |
| Downloadable project copy | backup |
| Isolated sample experience | demo |

## Privacy and not-found routes

| Copy | Words | Result |
| --- | ---: | --- |
| You can remove takes, site data, or your local license. | 10 | Listed as `data-removal` |
| Page not found | 3 | Direct recovery heading |
| This address does not match a page. | 7 | Direct recovery text |
| Go to Line Take Match | 5 | Clear recovery action |
